...
public:
    ...
    void Assign(InputIterator from, InputIterator to) {
        ...
        while (from != to) {
            Node* item = new Node(*from, nullptr);
            *node_ptr = item;
   ...
   //   Возвращает итератор на вставленный элемент
    Iterator InsertAfter(ConstIterator pos, const Type& value) {
        Node* new_pos = new Node(value,pos.node_->next_node );
    ...
    Iterator EraseAfter(ConstIterator pos) noexcept {
        ...
        delete temp; 
        size_=size_-1; // уменьшаем размер списка
    ...
    void swap(SingleLinkedList& other) noexcept {
        ...
        swap(this->head_.next_node,other.head_.next_node); 
 }
...
template <typename Type>
bool operator<=(const SingleLinkedList<Type>& lhs, const SingleLinkedList<Type>& rhs) {
    return (lhs. GetSize()== rhs.GetSize() &&std::equal(lhs.begin(), lhs.end(), rhs.begin()))|| std::lexicographical_compare(lhs.begin(),lhs.end(),rhs.begin(),rhs.end());
}
